...
//
// TODO: Here and in system_class, rename wrapper by putting it in a
// namespace within these files?
//
// TODO: Here and in system_class get the const qualifier on time?
template <typename T,
          void (T::*derivs)(const ode_system_stiff_class::state_type&,
                            ode_system_stiff_class::state_type&, double) =
          &T::derivs,
...
// TODO: Better, easier to use names here: These are going to be
// important.  Also with ode_system_class_generator.
template <typename T>
ode_system_stiff_class
ode_system_stiff_class_generator(typename T::pars_type pars) {
  T obj(pars);
  return stiff_wrapper<T>::make_ode_system(obj, Rcpp::wrap(pars));
}

template <typename T>
ode_system_stiff_class
ode_system_stiff_class_generator(typename T::pars_type pars, T obj) {
  return stiff_wrapper<T>::make_ode_system(obj, Rcpp::wrap(pars));
}
