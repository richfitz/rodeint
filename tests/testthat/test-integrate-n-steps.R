...
test_that("Time runs backwards", {
  ...
  for (category in stepper_categories()) {
    for (algorithm in stepper_algorithms(category)) {
      ...
      expect_that(integrate_n_steps(s, ode_system, y0, t0, dt, -1L),
                  throws_error("n must be nonnegative"))

      ## If n = 0, check output = input, regardless of sign of dt:
      expect_that(integrate_n_steps(s, ode_system, y0, t0, dt, 0),
                  is_identical_to(y0))
      expect_that(integrate_n_steps(s, ode_system, y0, t0, -dt, 0),
                  is_identical_to(y0))
      ...
      expect_that(integrate_n_steps(s, ode_system, y0, t0, 0, 1L),
                  throws_error("dt cannot be zero"))
      expect_that(integrate_n_steps(s, ode_system, y0, t0, 0, 0L),
                  throws_error("dt cannot be zero"))
    }
  }
})
...
test_that("Argument handling for errored input", {
  ...
  ## we're talking about.
  expect_that(integrate_n_steps(s, ode_system, y0, t0, stats::dt, n),
              throws_error("dt must be numeric"))

  expect_that(integrate_n_steps(s, ode_system, y0, "t0", dt, n),
  ...
  expect_that(integrate_n_steps(s, ode_system, y0, 0L, dt, n),
              is_identical_to(integrate_n_steps(s, ode_system, y0,
                                                0.0, dt, n)))
  expect_that(integrate_n_steps(s, ode_system, y0, t0, 1L, n),
              is_identical_to(integrate_n_steps(s, ode_system, y0,
                                                t0, 1.0, n)))
  ## And numerics in place of integers for n
  expect_that(integrate_n_steps(s, ode_system, y0, t0, dt, 20.0),
              is_identical_to(integrate_n_steps(s, ode_system, y0,
                                                t0, dt, 20L)))
  ## But not if they won't convert nicely to an integer
  expect_that(integrate_n_steps(s, ode_system, y0, t0, dt, 20.1),
              throws_error("n must be integer"))
})
