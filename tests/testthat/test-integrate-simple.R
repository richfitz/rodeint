...
test_that("Time ends at multiple of dt", {
  ...
  y_r <- integrate_simple(ode_system, y0, t0, t1, dt)
  expect_that(y_r, is_a("numeric"))

  y_r_s <- integrate_simple(ode_system, y0, t0, t1, dt, TRUE)
...
test_that("Time ends in the middle of a step", {
  ...
  y_r_s_1 <- integrate_simple(ode_system, y0, t0, t1,    dt, TRUE)
  y_r_s_2 <- integrate_simple(ode_system, y0, t0, t1+dt, dt, TRUE)
...
test_that("Time runs backwards", {
  ...
  expect_that(integrate_simple(ode_system, y0, t0, t1, -dt),
              throws_error("dt has the wrong sign"))
  expect_that(integrate_simple(ode_system, y0, t1, t0, dt),
              throws_error("dt has the wrong sign"))

  ## More corner cases: when t0 = t1, the sign of dt does not matter
  expect_that(integrate_simple(ode_system, y0, t0, t0, dt),
              is_identical_to(y0))
  expect_that(integrate_simple(ode_system, y0, t0, t0, -dt),
              is_identical_to(y0))
  ...
  expect_that(integrate_simple(ode_system, y0, t0, t1, 0),
              throws_error("dt cannot be zero"))
  expect_that(integrate_simple(ode_system, y0, t1, t0, 0),
              throws_error("dt cannot be zero"))
  expect_that(integrate_simple(ode_system, y0, t0, t0, 0),
              throws_error("dt cannot be zero"))
})
...
test_that("Argument handling for errored input", {
  ...
  ## we're talking about.
  expect_that(integrate_simple(ode_system, y0, t0, t1, stats::dt),
  ...
  expect_that(integrate_simple(ode_system, y0, 0L, t1, dt),
              is_identical_to(integrate_simple(ode_system, y0, 0.0, t1, dt)))
  expect_that(integrate_simple(ode_system, y0, t0, 1L, dt),
              is_identical_to(integrate_simple(ode_system, y0, t0, 1L, dt)))
  expect_that(integrate_simple(ode_system, y0, t0, t1, 1L),
              is_identical_to(integrate_simple(ode_system, y0, t0, t1, 1L)))
})
